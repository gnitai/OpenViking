...
- **服务端 API**：`POST /api/v1/search/grep`，body `{uri, pattern, case_insensitive}`。`uri` 默认 `wfs://session/{sessionId}/history`（覆盖所有 archive）；指定 `archiveId` 时收窄为 `wfs://session/{sessionId}/history/{archiveId}`
