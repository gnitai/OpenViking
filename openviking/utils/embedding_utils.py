    ...
    try:
        ...
        if content_type is None:
            ...
            if summary and effective_text_source in {"summary_first", "summary_only"}:
                ...
                # Prefer the text captured by the semantic stage; only hit AGFS
                # again when the prefetch is unavailable (re-enqueues, legacy
                # callers, or non-text-summary paths). Embedders apply their own
                # input guard, so no truncation here.
                try:
