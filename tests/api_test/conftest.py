...
def ensure_resources_dir(api_client):
    try:
        resp = api_client.fs_mkdir("wfs://resources")
        if resp.status_code in (200, 409):
            print("wfs://resources 目录已就绪")
        else:
            stat_resp = api_client.fs_stat("wfs://resources")
            if stat_resp.status_code == 200:
                print("wfs://resources 目录已存在")
            else:
                print(
                    f"警告: 无法确保 wfs://resources 存在 (mkdir={resp.status_code}, stat={stat_resp.status_code})"
                )
    except Exception as e:
