...
def test_create_tool_context_uses_extract_context_page_id_map():
    provider = AgentExperienceContextProvider(
        ...
        trajectory_uri="wfs://agent/agent_sample_9/memories/trajectories/album_release_party_discussion.md",
    )

    extract_context = provider.get_extract_context()
    extract_context.page_id_map.get_page_id(
        "wfs://agent/agent_sample_9/memories/trajectories/album_release_party_discussion.md"
    )
...
async def test_agent_experience_prefetch_includes_structured_read_results():
    ...
    provider.search_files = AsyncMock(
        return_value=["wfs://agent/agent_sample_9/memories/experiences/personal_experience_sharing_conversation_flow.md"]
    )
    ...
    provider._read_file_contents = {
        "wfs://agent/agent_sample_9/memories/experiences/personal_experience_sharing_conversation_flow.md": SimpleNamespace(
