...
"""Integration: the X-OpenViking-Callback-* headers reach the service layer.

Drives the *real* resources router through ASGI so the new ``Header`` route
parameters and the full request wiring are exercised (a malformed route
signature would fail at router registration / request handling, not at import).
"""
...
@pytest.mark.asyncio
async def test_callback_headers_reach_service():
    svc = _Service()
    app = _app(svc)
    resp = await _post(
        app,
        {
            "X-OpenViking-Callback-Url": "http://localhost:8080/callback",
            "X-OpenViking-Callback-Token": "CBTOKEN",
        },
    )
    assert resp.status_code == 200, resp.text
    assert svc.resources.calls, "service.add_resource was never called"
    assert svc.resources.calls[0]["callback_url"] == "http://localhost:8080/callback"
    assert svc.resources.calls[0]["callback_token"] == "CBTOKEN"


@pytest.mark.asyncio
async def test_no_callback_headers_means_none():
    svc = _Service()
    app = _app(svc)
    resp = await _post(app, {})
    assert resp.status_code == 200, resp.text
    assert svc.resources.calls[0]["callback_url"] is None
    assert svc.resources.calls[0]["callback_token"] is None
