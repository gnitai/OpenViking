...
Skills are stored at `wfs://agent/skills/`:
...
4. Store to `wfs://agent/skills/` path
...
```python
# List all skills
skills = client.ls("wfs://agent/skills/")
for skill in skills:
    print(f"{skill['name']}")

# Simple list (names only
names = client.ls("wfs://agent/skills/", simple=True)
print(names)
```
...
```bash
curl -X GET "http://localhost:1933/api/v1/fs/ls?uri=wfs://agent/skills/" \
  -H "X-API-Key: your-key"
```
...
```bash
# L0: Brief description
curl -X GET "http://localhost:1933/api/v1/content/abstract?uri=wfs://agent/skills/search-web/" \
  -H "X-API-Key: your-key"

# L1: Parameters and usage overview
curl -X GET "http://localhost:1933/api/v1/content/overview?uri=wfs://agent/skills/search-web/" \
  -H "X-API-Key: your-key"

# L2: Full skill documentation
curl -X GET "http://localhost:1933/api/v1/content/read?uri=wfs://agent/skills/search-web/" \
  -H "X-API-Key: your-key"
```
...
```python
client.rm("wfs://agent/skills/old-skill/", recursive=True)
```

**HTTP API**

```bash
curl -X DELETE "http://localhost:1933/api/v1/fs?uri=wfs://agent/skills/old-skill/&recursive=true" \
  -H "X-API-Key: your-key"
```
